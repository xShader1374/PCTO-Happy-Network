...
<div class="btn media-card text-secondary" data-bs-toggle="modal" data-bs-target="#mediaInspectorModal" onclick='updateModalInfo("<?= $media["nome"] ?>", "<?= $media["trailer_url"] ?>", "<?= addslashes($media["trama"]) ?>", "<?= $media["id"] ?>", "<?= $media["tipo"] ?>")'>
